...
kimi-doctor --gate perf-gate --dryrun --json       # plan closure without running
...
When implementing gates that read upstream results, use runner-provided `GateContext`
helpers on `GateRunOptions`: `getArtifact`, `getArtifacts({ limit, since })`,
`readArtifact(path)`. Runtime provenance is stored in `metadata.lineage`; declarative
audit queries use `metadata.dependsOn` at save time.

`topologicalSort()` in `src/gates/runner.ts` accepts `Gate[]` with `{ name, dependsOn }` — no `GraphNode` adapter. CLI and runner pass `resolveGateClosure(name).gates` from `src/gates/registry.ts`. Demo L2 loop: `strategy-performance` → `model-drift` (`examples/artifact-trading-loop.md`).
