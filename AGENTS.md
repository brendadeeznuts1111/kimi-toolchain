...
| Directory            | Contents                                                                                                                                                                                                                                                                                               |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `src/bin/`           | CLI entry points (29 registered bins) plus source-only tools (`kimi-doctor.ts`, `kimi-governance.ts`, `kimi-fix.ts`, `herdr-*.ts`, etc.). Registered tools are listed in `package.json` `bin`; several are source-only and must be run via `bun run src/bin/<tool>.ts` or the `kimi-toolchain` router. |
