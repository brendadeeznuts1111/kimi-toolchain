...
| **App Scaffold**        | `templates/scaffold/bunfig.toml`      | Template file (copied by `kimi-fix`)                                                | **No**                                  | N/A (installed per new project). For `bun install` flags and package-manager configuration see [Bun runtime scaffold flags](./bun-runtime-scaffold.md); for `bunfig.toml` runtime, test, serve, and `bun run` fields see [Bunfig config reference](./bunfig-config.md). |
