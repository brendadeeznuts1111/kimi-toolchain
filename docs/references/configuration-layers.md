...
| **Runtime Artifacts** | `thresholds.json`, `perf-report.html` | Generated by `kimi-doctor --train` / `--report` | **No** (runtime output) | **No** (per-project, `.gitignore`-d) — self-calibrating loop output; see [kimi-doctor.md](./kimi-doctor.md) § Effects pipeline. |
