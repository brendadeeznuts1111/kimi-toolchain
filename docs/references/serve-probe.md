...
**Precedence:** `PROBE_SERVER_PORT` / `PROBE_SERVER_HOST` env vars override TOML. When neither is set, runtime falls back to **9239** on `127.0.0.1` (`DEFAULT_PROBE_SERVER_PORT`).
...
```bash
# EXAMPLES_DASHBOARD_URL=http://127.0.0.1:3000
# HERDR_DASHBOARD_URL=http://127.0.0.1:18412
# PROBE_SERVER_HOST=127.0.0.1
# PROBE_SERVER_PORT=9239   # overrides [doctor.probe].port when set
```

Card auto-discovery also probes ports `3000`, `5678`, `8080` (examples) and `18412` (Herdr).
