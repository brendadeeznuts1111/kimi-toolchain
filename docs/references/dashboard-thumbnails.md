...
| `src/lib/herdr-dashboard-automation.ts` | `feedDashboardScreenshotPng` | *(none)* | `setScreenshotPng` only — encode on GET |
| `src/lib/herdr-dashboard-automation.ts` | `{ type: "screenshot", feed: true }` | *(none)* | Same deferred encode |
...
| `fetch` + `AbortSignal.timeout` | [fetch](https://bun.com/docs/runtime/fetch) | `probeDashboardThumbnail()` in automation gate |
