...
Generated by `src/harness/html-reporter.ts` — a standalone, self-contained HTML report with:

- **4 summary cards** — total benchmarks, pass rate, average latency, fastest operation (all computed from live `Metric[]` data)
- **Benchmark table** — per-operation metrics (registry key, symbol, actual ms, threshold ms, pass/fail) with proportional bar fills (safe/warn/danger coloring)
- **Skipped section** — when any benchmarks are unavailable at runtime (e.g. missing platform APIs), listed separately with skip reasons
- **Design system** — CSS custom properties in `:root` mirroring `Bun.color` macro output; dark theme (`#0d1117`), system-ui font, responsive grid layout
- **Footer** — generation timestamp and tool invocation reference

Zero CDN, zero framework, zero JavaScript — pure server-side string template.
