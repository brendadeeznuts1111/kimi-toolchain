...
| Effect audit + automated repair        | `invokeTool("kimi-heal", ["--fix", "--yes"])`              | Manual regex edits in source         | Centralized repair logic in `src/bin/kimi-heal.ts`; always `--dry-run` first in CI.            |
