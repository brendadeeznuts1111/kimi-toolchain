...
| 5        | **Herdr bridge**              | **Shipped** — `herdr-dashboard/server/bridge.ts`; `/api/canvases` `dashboardDeepLink`; Canvases tab **Examples** column; run-aware links via `artifactsRunFilter` → `?canvas=<manifestId>&runId=…` |
