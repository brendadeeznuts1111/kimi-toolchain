/** Dashboard shell — hub, PATH header, landing zone, canvas filter, shared helpers. */
...
function lineageBadgeHtml(source, deps) {
...
function lineageBadgeFromRow(row) {
  if (!row || (row.count ?? 0) === 0) return lineageBadgeHtml("none", 0);
  return lineageBadgeHtml(row.lineageSource || "none", row.dependencyCount ?? 0);
}

async function fetchLineageBadgeForGate(gate) {
  try {
    const payload = await fetchJson(`/api/artifacts/${encodeURIComponent(gate)}/lineage`);
    if (!payload.ok) return lineageBadgeHtml("none", 0);
    return lineageBadgeHtml(payload.lineageSource || "none", payload.dependencyCount ?? 0);
  } catch {
    return lineageBadgeHtml("none", 0);
  }
}

function artifactQuerySuffix(baseParams) {
...
function setCanvasFilterIds(cardIds, canvasId) {
