...
export async function apiPerfHarness(): Promise<Response> {
  ...
  const img = new Bun.Image(new Uint8Array([
    0x89,80,78,71,13,10,26,10,0,0,0,13,73,72,68,82,0,0,0,2,0,0,0,2,8,2,0,0,0,0xfd,0xd4,0x9a,
    0x73,0,0,0,18,73,68,65,84,8,0xd7,99,0xf8,0xcf,0xc0,0,
    2,12,0,0,9,0,1,0x35,0x8b,0x5a,0xc0,0,0,0,0,73,69,78,68,0xae,66,96,130,
  ]));
  ...
  return jsonResponse({
    ...
    philosophy: "Every Symbol-keyed effect produces a measurable artifact. Performance harness calls real (or fake) effect, measures elapsed, feeds into HTML table. Pure, deterministic, snapshot-testable.",
  });
}
