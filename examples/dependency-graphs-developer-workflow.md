...
Trading-domain sketch (not built-in yet):

```ts
export const modelDriftGate: Gate = {
  name: "model-drift",
  level: 2,
  dependsOn: ["strategy-performance"],
  run: async (opts) => {
    const perf = await opts.getArtifact?.("strategy-performance");
    // compute drift from perf.payload
    if (opts.saveArtifact) {
      await store.save("model-drift", result, {
        dependsOn: [{ gate: "strategy-performance", limit: 30 }],
      });
    }
  },
};
...
Built-in mapping: `card-probe` / `tls-compliance` → L1; `perf-gate` → L2;
`bunfig-policy` → L3.

## Alex the Trader (Dev Session)

1. Adds `model-drift` with `dependsOn: ["strategy-performance"]` and artifact `dependsOn` on save.
