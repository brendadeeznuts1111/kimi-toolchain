...
| Dashboard server / HTTP API      | `src/lib/herdr-dashboard-server.ts`                                                                  | `Bun.serve` + SSE; `/api/meta`, `/api/thumbnail`, `/api/doctor/gates`, `/api/metrics`, `/api/artifacts` (disk + sizes), `/api/probe/cards` (serve-probe proxy), widget routes                                                                                                                                                                                                                            |
...
| Gate artifact store              | `src/lib/artifact-store.ts`                                                                          | `.kimi/artifacts/{gate}/` envelopes with `metadata.resultSize`; `listEntries()` for probe list API; `--save-artifact` on gates / serve-probe                                                                                                                                                                                                                                                           |
| Gate system (dep graph)          | `src/gates/` (types, registry, runner, bunfig-policy, perf-gate, tls-compliance, card-probe)         | `dependsOn`-declared DAG, topological sort, `--run-gates` / `--gate <name>` / `--graph` in `kimi-doctor`; `resolveGateClosure()`, `runGatesWithDependencies()`, `generateGateGraph()`                                                                                                                  |
