...
| Cross-ref by intent (`@see`)      | [docs/references/namespace.md](docs/references/namespace.md#practical-see-ladder) § Practical `@see` ladder          | Lowest rung first — `@see dx` · `@see namespace-boundaries` (name collision table) · reference docs · schemas; runtime mirrors in `kimi-toolchain`, `finish-work`, `orchestrator`, `herdr` skills |
