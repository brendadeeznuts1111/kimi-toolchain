...
const policy = (Bun as unknown as { JSON5: { parse(s: string): unknown } }).JSON5?.parse(policyText) as PolicyDoc
  ?? JSON.parse(policyText) as PolicyDoc;
...
if (JSON_MODE) {
  console.log(JSON.stringify({ elapsedMs, errors: errors.length, warnings: warnings.length, violations }, null, 2));
...
if (violations.length === 0) {
  ...
  console.log(C.DIM(`  ${policyServices.length} services · ${secretKeyEntries.length} secret keys · ${Object.keys(Consumers).length} consumers`));
} else {
  ...
  const verdict = errors.length > 0
    ? C.RED(`✗ ${errors.length} error(s), ${warnings.length} warning(s) — fix before merging (${elapsedMs}ms)`)
