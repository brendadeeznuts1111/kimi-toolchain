...
function generateBashCompletion(data: CompletionData): string {
  ...
  const choiceMap = buildChoiceMap(data);
...
/**
 * Build a merged map of flag names to their choice values across global
 * and per-command flags. When the same flag appears in multiple commands
 * with different choice lists, keep the longest (most complete) list.
 */
function buildChoiceMap(data: CompletionData): Map<string, string[]> {
...
function generateZshCompletion(data: CompletionData): string {
  ...
  for (const flag of data.globalFlags) {
    ...
    const choices = choiceMap.get(`--${flag.name}`);
    const valueSpec = choices ? `: :(${choices.join(" ")})` : "";
    lines.push(
      `    '(${short} --${flag.name})${short}--${flag.name}[${flag.description.replace(/'/g, "'\\''")}]${valueSpec}'`
    );
  }
...
function generateFishCompletion(data: CompletionData): string {
  ...
  for (const flag of data.globalFlags) {
    ...
    const choices = choiceMap.get(`--${flag.name}`);
    const choicesArg = choices ? ` -a '${choices.join(" ")}'` : "";
    const value = flag.hasValue ? " -r" : "";
    lines.push(
      `complete -c bun -n '__fish_use_subcommand'${short} -l ${flag.name}${value}${choicesArg} -d '${desc}'`
    );
  }
  ...
  for (const [name, cmd] of Object.entries(data.commands)) {
    ...
    for (const flag of cmd.flags) {
      ...
      const value = flag.hasValue ? " -r" : "";
      lines.push(
        `complete -c bun -n '${condition}'${short} -l ${flag.name}${value}${choicesArg} -d '${desc}'`
      );
    }
  }
