...
function computeMetrics(upstreamCount: number): StrategyPerformanceMetrics {
  ...
  // Seed from upstream count so behaviour is deterministic per run.
  const seed = (upstreamCount + new Date().getDate()) * 7;
  ...
  return {
    dailyPnlBps: Math.round((pseudo(1) - 0.5) * 200), // -100 .. +100 bps
    sharpeRatio: Math.round(pseudo(2) * 300) / 100, // 0.00 .. 3.00
    winRate: Math.round(pseudo(3) * 60) / 100, // 0.00 .. 0.60
    maxDrawdownBps: Math.round(pseudo(4) * 600), // 0 .. 600 bps
...
export const strategyPerformanceGateDefinition: Gate = {
  ...
  level: 2,
  /** L1 deps are optional at runtime via getArtifacts; omit until L1 gates ship in scaffold. */
  dependsOn: [],
  parallel: true,
