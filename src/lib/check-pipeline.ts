...
export async function buildSteps(
  ...
  if (await isKimiToolchainRepo(projectRoot)) {
    ...
    if (pathExists(new URL("scripts/check-env-drift.ts", Bun.pathToFileURL(`${projectRoot}/`)).pathname)) {
      steps.push({
    ...
    if (
      pathExists(new URL("scripts/validate-release-ssot.ts", Bun.pathToFileURL(`${projectRoot}/`)).pathname)
    ) {
  ...
  if (options.fast && (await isKimiToolchainRepo(projectRoot))) {
    ...
    if (pathExists(new URL("scripts/autophagy-scan.ts", Bun.pathToFileURL(`${projectRoot}/`)).pathname)) {
      steps.push({
        name: "autophagy:scan",
    ...
    if (
      pathExists(new URL("scripts/bun-hygiene-audit.ts", Bun.pathToFileURL(`${projectRoot}/`)).pathname)
    ) {
      steps.push({
  ...
  if (options.fast && pathExists(new URL("scripts/scan.ts", Bun.pathToFileURL(`${projectRoot}/`)).pathname)) {
