/**
 * herdr-dashboard/automation/automation.ts — Bun.WebView screenshot, click, and CDP probes for the dashboard.
 *
...
export const AGENT_ROW_SELECTOR = `${AGENTS_BODY_SELECTOR} tr.agent-row`;
export const AGENT_ATTACH_SELECTOR = `${AGENT_ROW_SELECTOR} button[data-action="attach"]`;
...
export async function runHerdrDashboardAutomation(
  ...
  try {
    ...
    const scrollBudgetMs = Math.min(options.readyTimeoutMs ?? 10_000, 5_000);

    let clickAttachOk: boolean | undefined;
    if (options.clickAttach && agentRows > 0) {
      try {
        const attachReady = await scrollToDashboardSelector(view, AGENT_ATTACH_SELECTOR, {
          timeoutMs: scrollBudgetMs,
        });
        if (!attachReady) {
          clickAttachOk = false;
        } else {
    ...
    const wantsCapture = Boolean(options.outputPath || options.thumbnailPath);
    let png: Uint8Array = new Uint8Array(0);
    if (wantsCapture) {
      await scrollToDashboardAgentsBody(view, { timeoutMs: scrollBudgetMs });
      png = await webViewScreenshotBytes(view);
    }

    let outputPath: string | undefined;
    if (options.outputPath && png.byteLength > 0) {
    ...
    if (options.thumbnailPath && png.byteLength > 0 && bunImageSupported()) {
