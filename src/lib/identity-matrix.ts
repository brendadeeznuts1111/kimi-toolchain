...
function isAuditRecord(value: unknown): value is IdentityAuditRecord {
  return (
    !!value &&
    typeof value === "object" &&
    (value as IdentityAuditRecord).schemaVersion === IDENTITY_MATRIX_SCHEMA_VERSION &&
    typeof (value as IdentityAuditRecord).id === "string" &&
    typeof (value as IdentityAuditRecord).repoPath === "string"
  );
}
...
export async function readIdentityAudit(projectRoot: string): Promise<IdentityAuditRecord[]> {
  return readNdjsonFile(identityAuditPath(projectRoot), isAuditRecord);
}
