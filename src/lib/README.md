...
| **Governance**      | `r-score.ts`, `governance-check.ts`, `readme-sync.ts`                                                                                                                     | R-Score calculation, license/CONTRIBUTING checker, README drift (`kimi-governance.ts` bin) |
...
| **Cloudflare**      | `cloudflare-access.ts`, `cloudflare-access-policy.ts`, `cloudflare-integration-status.ts`, `dx-cloudflare-config.ts`                                                      | Cloudflare Access API, policy diff/plan/apply, local DX/MCP inventory, dashboard contract  |
...
| **Config**          | `mcp-config.ts`, `kimi-config-audit.ts`, `dx-github-alignment.ts`, `dx-cloudflare-config.ts`, `test-gates.ts`                                                             | MCP/Kimi/DX configuration audits and test gate configs                                     |
| **Property tables** | `toml-property-table.ts`, `property-table-run.ts`, `property-table-options.ts`, `property-table-renderer.ts`, `markdown-table.ts`, `table-schema.ts`, `url-decomposer.ts` | `dx:table` extract/inventory — see [docs/dx-table.md](../../docs/dx-table.md)              |
