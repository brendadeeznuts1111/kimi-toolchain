...
| **Config**     | `mcp-config.ts`, `kimi-config-audit.ts`, `test-gates.ts`, `test-runtime.ts`, `testing-docs-lint.ts`, `artifacts.ts` | MCP config, test tiers, Bun test SSOT, testing doc audit, artifacts  |
...
| **Sync**       | `desktop-sync.ts`                                                                                                   | Desktop sync, hash verification, manifest, baseline restore, metrics |
