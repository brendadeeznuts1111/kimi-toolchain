...
import { buildDecisionGraph, type DecisionGraph } from "../decision-graph.ts";
...
export interface DecisionQueryService {
  ...
  readonly why: (
    decisionId: string,
    projectRoot?: string
  ) => Effect.Effect<Awaited<ReturnType<typeof buildWhyReport>> | null, never>;
...
const DecisionQueryLive = Layer.succeed(DecisionQuery, {
  ...
  graph: (traceId, projectRoot) =>
    Effect.gen(function* () {
      ...
      return yield* Effect.promise(() => buildDecisionGraph(traceId, decisions));
    }),
  ...
  scoreAll: (projectRoot) =>
    scoreAllDecisionsEffect({ projectRoot }).pipe(
      Effect.catchAll(() => Effect.succeed({ scores: {}, total: 0 }))
    ),
  scoreOne: (decision, projectRoot) =>
    Effect.gen(function* () {
      ...
      return scoreDecision(decision, { decisions: all, failures });
    }),
});
