...
import { artifactIdentityEnv } from "./artifact-store.ts";
...
export function startHerdrAgent(
  ...
  const env: Record<string, string> = {
    ...artifactIdentityEnv(options.workspaceId, config.session),
    ...options.env,
  };
    ...
    if (pane.role === "primary" && pane.agent) {
      ...
      if (!already) {
        const started = startHerdrAgent(config, pane.agent, argv, {
          ...
          env: {
            HERDR_ROLE: "primary",
            ...artifactIdentityEnv(workspaceId, config.session),
            ...pane.env,
          },
        });
