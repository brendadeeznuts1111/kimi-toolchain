...
async function shouldSkipAppend(
  ...
  const records = await readNdjsonFile<OptimizerHealthTrendRecord>(path);
...
export async function readOptimizerHealthTrend(
  ...
  return readNdjsonFile<OptimizerHealthTrendRecord>(optimizerHealthTrendPath(projectRoot));
}
