...
function isValidManifestId(value: string, violations: string[], path: string): boolean {
...
export function lintCanonicalReferencesLinkTables(tables: CanonicalReferencesLinkTables): string[] {
  ...
  for (const ref of tables.ecosystem) {
    ...
    if (!ref.id || !isValidManifestId(ref.id, violations, `${path}.id`)) continue;
    ...
    if (ref.repoId !== undefined) {
      if (!isValidManifestId(ref.repoId, violations, `${path}.repoId`)) continue;
  ...
  for (const doc of tables.localDocs) {
    ...
    if (!doc.id || !isValidManifestId(doc.id, violations, `${path}.id`)) continue;
  ...
  for (const repo of tables.repos) {
    ...
    if (!repo.id || !isValidManifestId(repo.id, violations, `${path}.id`)) continue;
