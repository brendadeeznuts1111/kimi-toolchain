...
function buildPaneRunShellPayload(paneId: string, command: string): string {
  const parts: string[] = [];
  const path = resolveHerdrPanePath();
  if (path) {
    const escapedPath = path.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
    parts.push(`export PATH="${escapedPath}"`);
  }
  const identityExports = buildPaneIdentityExports(paneId);
  if (identityExports) parts.push(identityExports);
  parts.push(command);
  return parts.join("; ");
}
...
export function paneRunSync(
  ...
  const payload = buildPaneRunShellPayload(paneId, command);
