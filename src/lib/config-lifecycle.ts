...
function isLifecycleRecord(value: unknown): value is ConfigLifecycleRecord {
  return (
    !!value &&
    typeof value === "object" &&
    (value as ConfigLifecycleRecord).schemaVersion === CONFIG_LIFECYCLE_SCHEMA_VERSION &&
    typeof (value as ConfigLifecycleRecord).id === "string" &&
    typeof (value as ConfigLifecycleRecord).timestamp === "string" &&
    typeof (value as ConfigLifecycleRecord).constant === "string"
  );
}
...
export async function readConfigLifecycle(projectRoot: string): Promise<ConfigLifecycleRecord[]> {
  return readNdjsonFile(configLifecyclePath(projectRoot), isLifecycleRecord);
}
