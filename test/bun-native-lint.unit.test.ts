...
describe("bun-native-lint", () => {
  ...
  test("engine source is not self-flagged for process-env", async () => {
    const { scanRepo, mergeConfig } = await import("../src/lib/bun-native-lint.ts");
    const config = mergeConfig({
      rules: {
        "banned-import": "off",
        "banned-require": "off",
    ...
    const violations = await scanRepo(join(import.meta.dir, ".."), config);
    expect(violations.some((v) => v.file === "src/lib/bun-native-lint.ts")).toBe(false);
  });
});
