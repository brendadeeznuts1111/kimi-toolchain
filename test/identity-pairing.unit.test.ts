    ...
    await run(
      Effect.gen(function* () {
        ...
        const result = yield* Effect.either(
          identity.verifyCsrf(csrf1, session2.id)
        );
  ...
  test("verifyCsrf fails for token from different secret", async () => {
    ...
    await run(
      Effect.gen(function* () {
        ...
        const result = yield* Effect.either(
          identity.verifyCsrf(csrfFromOther, "session-x")
        );
...
describe("identity-service > password hardening", () => {
  test("hashPassword produces different hashes for same input (salt)", async () => {
    await run(
      Effect.gen(function* () {
        const identity = yield* Identity;

        const h1 = yield* identity.hashPassword("mypassword");
        const h2 = yield* identity.hashPassword("mypassword");
  ...
  test("verifyPassword succeeds for correct password regardless of hash variant", async () => {
    await run(
      Effect.gen(function* () {
        const identity = yield* Identity;

        const hash = yield* identity.hashPassword("correct-pass");
        const ok = yield* identity.verifyPassword("correct-pass", hash);
  ...
  test("verifyPassword fails for wrong password", async () => {
    await run(
      Effect.gen(function* () {
        ...
        const hash = yield* identity.hashPassword("correct-pass");
        const ok = yield* identity.verifyPassword("wrong-pass", hash);
