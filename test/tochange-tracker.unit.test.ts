...
describe("tochange-tracker", () => {
  ...
  test(
    "auditPeekAdoption uses registry probes with zero pending",
    async () => {
      const report = await auditPeekAdoption(REPO_ROOT);
      expect(report.ok).toBe(true);
      expect(report.registryPending).toHaveLength(0);
      expect(report.registryImplemented.length).toBeGreaterThan(0);
...
console.log("ok");`
  ...
  test(
    "scanDirectStreamReads finds no feature-code bypasses",
    async () => {
