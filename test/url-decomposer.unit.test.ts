...
describe("url-decomposer", () => {
  ...
  test("decomposeUrl normalizes Unicode IDN hostnames to ASCII Punycode", () => {
    expect(decomposeUrl("https://mañana.com/v1").hostname).toBe("xn--maana-pta.com");
    expect(decomposeUrl("https://☃-⌘.com/").hostname).toBe("xn----dqo34k.com");
    expect(normalizeHostnameAscii("MAÑANA.COM")).toBe("xn--maana-pta.com");
  });

  test("decodePunycodeHostname decodes xn-- labels for display", () => {
    expect(decodePunycodeHostname("xn--maana-pta.com")).toBe("mañana.com");
    expect(decodePunycodeHostname("api.xn--maana-pta.com")).toBe("api.mañana.com");
    expect(decodePunycodeHostname("api.example.com")).toBe("api.example.com");
  });

  test("encodePunycodeLabel and decodePunycodeLabel roundtrip per label", () => {
    expect(encodePunycodeLabel("mañana")).toBe("maana-pta");
    expect(decodePunycodeLabel("maana-pta")).toBe("mañana");
    expect(punycodeLabelToAsciiLabel("mañana")).toBe("xn--maana-pta");
    expect(punycodeLabelToAsciiLabel("example")).toBe("example");
  });

  test("punycode helpers expose Bun reference anchors", () => {
    expect(BUN_PUNYCODE_TO_ASCII_DOC_URL).toBe(
      "https://bun.sh/reference/node/punycode/toASCII#node:punycode.toASCII"
    );
    expect(BUN_PUNYCODE_ENCODE_DOC_URL).toBe(
      "https://bun.sh/reference/node/punycode/encode#node:punycode.encode"
    );
    expect(BUN_PUNYCODE_DECODE_DOC_URL).toBe("https://bun.sh/reference/node/punycode/decode");
  });
