...
describe("buildConstants", () => {
  it("should load positive compile-time tuning values from bunfig define", () => {
    expect(EMBEDDING_DIM).toBe(384);
    expect(DECISION_SCORE_WINDOW_DAYS).toBe(7);
    expect(CLUSTER_SIMILARITY_THRESHOLD).toBe(0.55);
    expect(HOOK_VERIFIER_MAX_CYCLES).toBeGreaterThan(0);
  ...
  it("should enforce hook cycle limit from define", () => {
    const ok = verifyHookCycleLength(HOOK_VERIFIER_MAX_CYCLES);
    expect(ok.ok).toBe(true);
    const bad = verifyHookCycleLength(HOOK_VERIFIER_MAX_CYCLES + 1);
