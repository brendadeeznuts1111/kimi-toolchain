...
/** Probe server spawn tests exceed pre-commit test:changed global timeout (1500ms). */
...
describe("card-probe-server", () => {
    ...
    async () => {
      ...
      try {
        ...
        const cardsBody = (await cards.json()) as {
          ok: boolean;
          cards: Array<{ cardId: string; source: string; status: string }>;
          total: number;
        ...
        const configStatusCard = cardsBody.cards.find((c) => c.cardId === "config-status");
        expect(configStatusCard).toBeDefined();
        expect(configStatusCard?.source).toBe("config-status");
        expect(["pass", "fail", "skip"]).toContain(configStatusCard!.status);
    ...
    async () => {
      await withTempDir("card-probe-server-artifacts-", async (dir) => {
        ...
        try {
          ...
          expect(gatesBody.gates).toContain("config-status");
          expect(gatesBody.count).toBe(gatesBody.gates.length);

          const configList = await fetch(`${handle.url}/api/artifacts/config-status?limit=1`);
          expect(configList.status).toBe(200);
          const configListBody = (await configList.json()) as {
            ok: boolean;
            gate: string;
            count: number;
            files: Array<{ path: string; timestamp: string | null }>;
          };
          expect(configListBody.ok).toBe(true);
          expect(configListBody.gate).toBe("config-status");
          expect(configListBody.count).toBeGreaterThanOrEqual(1);
          expect(configListBody.files.length).toBe(1);
          expect(configListBody.files[0]?.path).toMatch(/^\.kimi\/artifacts\/config-status\//);

          const configLatest = await fetch(`${handle.url}/api/artifacts/config-status/latest`);
          expect(configLatest.status).toBe(200);
          const configLatestBody = (await configLatest.json()) as {
            ok: boolean;
            gate: string;
            payload: { tool: string; aligned: boolean };
          };
          expect(configLatestBody.ok).toBe(true);
          expect(configLatestBody.gate).toBe("config-status");
          expect(configLatestBody.payload.tool).toBe("config-status");
          expect(typeof configLatestBody.payload.aligned).toBe("boolean");
  ...
  test(
    "refresh response includes artifactPath when saveArtifact is enabled",
    async () => {
